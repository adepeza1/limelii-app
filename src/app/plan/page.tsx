...
function SkeletonCard() {
  return (
    <div className="w-full aspect-[33/38] rounded-[20px] bg-[#1c1c1a] animate-pulse" />
  );
}
...
export default function PlanPage() {
  ...
  return (
    <div className="bg-[#0f0f0d] min-h-screen max-w-5xl mx-auto relative">
      <div className="h-[44px]" />

      {/* Header */}
      <div className="px-5 pt-4 pb-6">
        <span className="text-[#c5f135] font-bold text-xl tracking-tight font-[family-name:var(--font-poppins),sans-serif]">
          limelii
        </span>
        <h1 className="text-white text-[2.25rem] font-bold leading-tight mt-3">
      ...
      <div className="px-5 flex flex-col gap-7 pb-6">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2.5 rounded-full text-sm font-medium transition-colors ${
                location === "__current__"
                  ? "bg-[#c5f135] text-[#0f0f0d]"
                  : "bg-[#1c1c1a] text-[#d1d5db]"
              }`}
            >
        ...
        <div>
          ...
          <div className="flex bg-[#1c1c1a] rounded-full p-1">
            {SETTINGS.map((s) => (
              <button
                key={s}
      ...
      <div className="px-5 pb-8 flex flex-col items-center gap-3">
        {!searched && (
          <p className="text-[#6b7280] text-sm">Choose your filters above</p>
        )}
        <button
          onClick={handleSearch}
          disabled={loading}
          className="w-full bg-[#c5f135] text-[#0f0f0d] font-bold text-base rounded-full py-4 flex items-center justify-center gap-3 disabled:opacity-60 active:opacity-80 transition-opacity"
        >
          {loading ? "Finding..." : "Find experiences"}
          {!loading && (
            <span className="w-7 h-7 bg-[#0f0f0d] rounded-full flex items-center justify-center">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M2 7h10M8 3l4 4-4 4" stroke="#c5f135" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </span>
          )}
        </button>
